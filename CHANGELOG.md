...
- Fixed the SMB service failure that prevented the `[shared]` resource from being mounted.
- Added the Debian `samba-vfs-modules` package to the custom Samba image so `vfs_full_audit` and `full_audit.so` are available at runtime.
- Added installation-time verification that both SMB containers contain the `full_audit` module before deployment is declared successful.
- Extended release validation to reject SMB images that omit `samba-vfs-modules` or installers that omit the runtime module check.
- Preserved all validated services, networks, ports, credentials, logging routes and v2.1.2 DVWA telemetry safeguards unchanged.
- Regenerated the complete internal project archive and release checksums from one coherent source tree.
