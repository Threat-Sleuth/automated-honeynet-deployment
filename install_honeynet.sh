...
create_log_directories() {
  ...
  # mitmproxy runs as UID/GID 1000 and writes DVWA events directly to /logs.
  # Keep the hardened 0775 mode, but explicitly assign ownership so HTTP
  # telemetry is writable on every clean deployment.
  chown -R 1000:1000 \
    "${PROJECT_DIR}/logs/dvwa_normalidad" \
    "${PROJECT_DIR}/logs/dvwa_pentesting" \
    "${PROJECT_DIR}/mitmproxy" 2>/dev/null || true
}
...
verify_smb_audit_module() {
  local container module_path
  log "Verifying Samba full_audit modules..."
  for container in smb-normalidad smb-pentesting; do
    module_path=$(docker exec "$container" sh -c 'find /usr -type f -name full_audit.so -print -quit' 2>/dev/null || true)
    [[ -n "$module_path" ]] || fail "${container} does not contain full_audit.so; the samba-vfs-modules package is missing."
  done
  ok "Both SMB containers contain the Samba full_audit module."
}

verify_telemetry_writability() {
  local container
  log "Verifying mitmproxy telemetry-directory writability..."
  for container in reverse-proxy-normalidad reverse-proxy-pentesting; do
    docker exec "$container" sh -c 'test -w /logs && : > /logs/.write-test && rm -f /logs/.write-test' \
      || fail "${container} cannot write to its /logs mount."
  done
  ok "Both mitmproxy instances can write HTTP telemetry."
}
