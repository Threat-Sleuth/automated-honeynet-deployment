...
# Require the Samba full_audit runtime dependency and installer verification.
grep -Eq 'samba([[:space:]\\]+|[[:space:]].*)samba-vfs-modules|samba-vfs-modules([[:space:]\\]+|[[:space:]].*)samba' "$PROJECT/smb-custom/Dockerfile" \
  || { echo '[FAIL] SMB image does not install samba-vfs-modules.' >&2; exit 1; }
grep -Fq 'verify_smb_audit_module' install_honeynet.sh \
  || { echo '[FAIL] Installer lacks SMB full_audit runtime verification.' >&2; exit 1; }
grep -Fq 'full_audit.so' install_honeynet.sh \
  || { echo '[FAIL] Installer does not verify full_audit.so.' >&2; exit 1; }
