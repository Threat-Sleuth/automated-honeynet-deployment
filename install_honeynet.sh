...
prepare_project() {
  local src archive service_unit host_ip
  src="$(script_dir)"
  ...
  # Load the previously stored HOST_IP before invoking Compose on an
  # existing deployment. This prevents Compose warnings during upgrades.
  if [[ -r "$ENV_FILE" ]]; then
  ...
  local compose_service cid state all_running
  while (( SECONDS < deadline )); do
    all_running=1
    for compose_service in "${expected_services[@]}"; do
      cid="$(cd "$PROJECT_DIR" && docker compose ps -q "$compose_service")"
      if [[ -z "$cid" ]]; then
        all_running=0
        continue
      fi
  ...
  for compose_service in "${expected_services[@]}"; do
    cid="$(cd "$PROJECT_DIR" && docker compose ps -q "$compose_service")"
    if [[ -z "$cid" ]]; then
      failed_services+=("${compose_service}:not-created")
      continue
    fi
    state="$(docker inspect -f '{{.State.Status}}' "$cid" 2>/dev/null || true)"
    [[ "$state" == "running" ]] || failed_services+=("${compose_service}:${state:-unknown}")
  done
...
' >&2
  ...
  # A previous broken installation may have left this path as a directory
  # or symlink. Remove it before installing the unit as a regular file.
