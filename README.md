...
**Current release: 2.1.3.** This corrective release adds the missing Debian `samba-vfs-modules` package to the custom Samba image and verifies that `full_audit.so` exists in both SMB containers during installation. It preserves the v2.1.2 architecture, ports, credentials and all other service configurations.
...
7. creates all log and Fluentd buffer directories and assigns the DVWA log mounts to the mitmproxy runtime UID;
...
12. verifies that both mitmproxy containers can create files in their `/logs` mounts;
13. installs and enables `honeynet.service`.
...
The canonical credential table is maintained in [`CREDENTIALS.md`](CREDENTIALS.md). Key defaults are: DVWA `admin/password`, FTP `ftpuser/password`, SSH `root/password`, SMB `smbuser/password`, mail accounts `usuario1` and `usuario2` with password `password`, separate PostgreSQL credentials for each environment, and distinct wg-easy administrator passwords.

Portainer has no predefined password; create its administrator during first access. FastAPI seed users contain bcrypt hashes rather than a shared plaintext password.

These credentials are deliberate laboratory defaults and must never be used outside an isolated, authorized environment.
...
First confirm that the required VFS module exists in both containers:

```bash
sudo docker exec smb-normalidad sh -c 'find /usr -name full_audit.so -type f -print -quit'
sudo docker exec smb-pentesting sh -c 'find /usr -name full_audit.so -type f -print -quit'
```

Then generate a real file operation rather than only checking that port 445 is open, and inspect:
